...
async def song(client, message: Message):
    ...
    if not urlissed:
        ...
        return
    pablo = await client.send_message(message.chat.id, f"**🔎 جاري تحميل : @uwrwe ** `{urlissed}`")
...
async def vsong(client, message: Message):
    ...
    pablo = await client.send_message(message.chat.id, f"**🔎 يبحث عن : @uwrwe** `{urlissed}`")
    if not urlissed:
    ...
    try:
        ...
        await event.edit(event, f"**التحميل فشل : @uwrwe** \n `{str(e)}`")
        return
    c_time = time.time()
